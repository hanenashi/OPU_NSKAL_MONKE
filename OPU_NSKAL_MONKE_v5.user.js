...
import { createSettingsPanel, createEditWindow, addFileToList, openCropWindow, updatePreview, generateOutputFormat } from './ui.js';
...
const UI = {
                ...
                <script>
                    ...
                    window.addEventListener('message', (event) => {
                        if (event.data.type === 'croppedFile') {
                            console.log('Message received in crop window:', event.data);
                            setTimeout(() => {
                                console.log('Closing crop window');
                                window.close();
                            }, 1000); // Delay to ensure the message is processed
                        }
                    });
                </script>
            </body>
    ...
    const insertNskalButtons = (toolsDiv) => {
        ...
        const settingsPanel = createSettingsPanel(settings, (newSettings) => {
            Storage.saveSettings(newSettings);
            settings = newSettings;
            updatePreview(galleryLinks, settings);
        });
    ...
    const uploadFiles = async (files) => {
        ...
        if (uploadedLinks.length > 0) {
            const customTag = settings.customTag || '<p>';
            const imgTags = uploadedLinks.map(link => generateOutputFormat(link, settings.toggles)).join(`\n\n${customTag}\n\n`);
    ...
    const openEditWindow = () => {
        ...
        fileInput.onchange = event => {
            ...
            if (files.length) {
                const imgWindow = createEditWindow();
